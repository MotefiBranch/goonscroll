...
export async function fetchRule34({ tags = '', page = 0, limit = 42, blacklist = [], credentials = {} }) {
  ...
  // If user has Rule34 API credentials configured, use official JSON API
  ...
  // Fallback: Web list parser (no API key required)
  const pid = page * 42;
  const webUrl = `https://rule34.xxx/index.php?page=post&s=list&tags=${encodeURIComponent(queryTags)}&pid=${pid}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(`Rule34 request failed: ${response.status}`);
  }
