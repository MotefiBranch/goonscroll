...
export async function fetchXbooru({ tags = '', page = 0, limit = 40, blacklist = [] }) {
  const queryTags = buildQueryTags(tags, blacklist);
  const encodedTags = encodeURIComponent(queryTags);
  const url = `https://xbooru.com/index.php?page=dapi&s=post&q=index&json=1&tags=${encodedTags}&pid=${page}&limit=${limit}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(`Xbooru API error: ${response.status}`);
  }
  ...
  const items = posts.map(post => {
    ...
    const mediaUrl = post.file_url || `https://xbooru.com/images/${post.directory}/${post.image}`;
    const previewUrl = post.preview_url || `https://xbooru.com/thumbnails/${post.directory}/thumbnail_${post.image}`;
